...
def paste_from_premiere():
    if platform.system() == "Darwin":
        ...
        if premiere_data_string is None:
            print("Premiere data not found on clipboard.")
            return None
        ...
        try:
            ...
            print("Could not parse ArbVideoComponentParam object id")
            exit()
...
def convert_button_clicked():
    if ae_clipboard_data is None or premiere_clipboard_data is None:
        print("Error, the button should not have been clickable")
        return None
...
premiere_status_label.grid(column=0, row=1)
...
ae_status_label.grid(column=0, row=1)
